...
app.post('/users', async function(req, res) {
	...
	let response = await helper.getRegisteredUser(username, orgName, password,role, true);
...
app.post('/channels/:channelName/chaincodes/:chaincodeName', async function(req, res) {
	...
	var fcn = req.body.fcn;
	var args = req.body.args;
	if(fcn==="bookWorkSpace"){
		var startTime=req.body.StartTime 
		var endTime=req.body.EndTime
		var iStartTime=moment(startTime, "D/M/YYYY H:mm").valueOf();
		var iEndTime=moment(endTime, "D/M/YYYY H:mm").valueOf()
		var iCurTime=moment().valueOf()
		if(iStartTime>iEndTime ||iStartTime<=iCurTime){
			
			var msg="Invlaid Dates"
				logger.debug("invalid dates")
				res.send(msg)
				return
		}
		if(iStartTime<=iCurTime+3600000){
			var msg="Very less time to Process the request"
				res.send(msg)
				return
		}
		var tim=`{"startTime":`+iStartTime+`,"endTime":`+iEndTime+`}`
		args.push(tim)
	}
	logger.debug('channelName  : ' + channelName);
	logger.debug('chaincodeName : ' + chaincodeName);
...
// consensus Approval or deny
app.get('/channels/:channelName/chaincodes/:chaincodeName/:txnId/',async function(req,res){
logger.debug("----------------entering consensus in app.js----------------------------")
	var channelName=req.params.channelName;
	var chaincodeName=req.params.chaincodeName;
	var trxnId=req.params.txnId
	var fcn=req.query.fcn
	var target=req.query.targets
	...
	logger.debug(
		'================ approve using txID and occupy cubicle ======================'
	);
	logger.debug('channelName : ' + req.params.channelName);
	let peer = req.query.peer;
	if (!trxnId) {
	...
	let TxId=await query.queryAndInvoke(peer, trxnId, req.username, req.orgname,channelName, chaincodeName, permission,fcn)
...
// Query Write sets Get Transaction by Transaction ID
app.get('/channels/:channelName/transactions/:trxnId/writeSet', async function(req, res) {
	logger.debug('================ GET TRANSACTION BY TRANSACTION_ID ======================');
	logger.debug('channelName : ' + req.params.channelName);
	let trxnId = req.params.trxnId;
	let peer = req.query.peer;
	...
	let message = await query.getWriteSetTransactionByID(peer, req.params.channelName, trxnId, req.username, req.orgname);
	res.send(message);
});
// app.get('/insert/:txId/:org',async function(req,res){
...
//   var sql = "INSERT INTO BookingRequest (txid, org) VALUES (txId, 'org')";
